...
function ProjectList() {
  ...
  const { data } = useQuery({
    ...
    queryFn: () =>
      getCategoryProjects(status ? status : "ongoing", page ? page : "1"),
  });
    ...
    <>
        ...
        <div className="w-full max-w-screen-lg grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 mt-20">
          {data?.data?.map((project: any) => (
            <Card
              ...
              name={project?.name}
              route={`/projects/ongoing-projects/${project?._id}`}
              status={
      ...
      <div className="mt-10 w-full justify-center items-center flex">
        <Pagination
          ...
          variant="bordered"
          onChange={(value) => {
            router.push(`ongoing-projects?status=ongoing&page=${value}`);
          }}
        />
      </div>
    </>
